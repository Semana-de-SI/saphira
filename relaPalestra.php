						...
						<div class="card-body">
							...
							<?php
								if ($_POST['subdivisao'] == "*") {
									...
									<input type="submit" name="Baixar" class="btn btn--radius-2" style="background-color: <?php echo $_SESSION['corfundo'] ?>;" value="Baixar Relatorio - Lista de Palestras que Cada Aluno Foi" /> <?php
								}
								else {
								...
								if ($_POST['subdivisao'] != "*") {
									...
									if (mysqli_num_rows($result) >= 1) {
										$row3 = mysqli_fetch_assoc($result);
										?><h1 class="BemVindo"><?php echo $row3['Nome'] ?> (<?php echo $row3['Quantidade_presentes'] ?>)</h1><?php
										while ($row = mysqli_fetch_assoc($result)) {
