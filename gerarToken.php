...
<?php
	...
	if (
		isset($_POST['subdivisao']) &&
		isset($_POST['token']) &&
		!empty($_POST['dataExpiraToken']) &&
		isset($_POST['Enviar'])
	) {
		...
		if (mysqli_num_rows($result) >= 1) {
			...
			if ($row['dataExpiraToken'] > date('Y-m-d H:i:s')) {
				?> <script type='text/javascript'>alert('Token que estava valido da palestra <?php $row["Nome"] ?> deletado.')</script> <?php
			}
			$sql = "UPDATE saphira_subdivisoes SET token= '', dataExpiraToken= '1900-01-01 00:00:00' WHERE token != ''";
			$result = mysqli_query($link, $sql);
		}

		$sql =
			"UPDATE saphira_subdivisoes SET token= '" .
			$_POST['token'] .
			"', dataExpiraToken= '" .
			str_replace('T', ' ', $_POST['dataExpiraToken']) .
			"' WHERE ID_subdivisoes = '" .
			$_POST['subdivisao'] .
			"'";
		$result = mysqli_query($link, $sql);
	}
						...
						<form method="POST" id="form">
							<?php
								...
								if (mysqli_num_rows($result) >= 1) {
									...
									if ($row['dataExpiraToken'] > date('Y-m-d H:i:s')) {
										?><h2 style="font-size: 15px; text-align: center; margin-top: 0; margin-bottom: 30px;">Token <span style="text-decoration: underline; text-decoration-color: green;">Valido</span></h2>
										<h2 style="font-size: 15px; text-align: center; margin-top: 0; margin-bottom: 30px;">Palestra: <b><?= $row['Nome'] ?></b></h2>
										...
										?><h2 style="font-size: 15px; text-align: center; margin-top: 0; margin-bottom: 30px;">Token <span style="text-decoration: underline; text-decoration-color: red;">Expirado</span></h2>
										<h2 style="font-size: 15px; text-align: center; margin-top: 0; margin-bottom: 30px;">Palestra: <b><?= $row['Nome'] ?></b></h2>
									...
									?> <h1 class="title" style="font-size: 30px; margin-bottom: 20px;">Nenhuma token encontrado</h1> <?php
								}
							?>
